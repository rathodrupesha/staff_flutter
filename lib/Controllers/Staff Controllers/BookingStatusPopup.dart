...
class BookingStatusPopup extends StatefulWidget {
  BookingStatusPopup(this.id,{Key? key}) : super(key: key);

  String id;
  @override
  _BookingStatusPopupState createState() => _BookingStatusPopupState();
}

class _BookingStatusPopupState extends State<BookingStatusPopup> implements OnResponseCallback{
  String dropdownValue = Translations.current!.strServiceAccepted;
                    ...
                    child: Column(
                      ...
                      children: [
                        WidgetUtils().simpleTextViewWithGivenFontSize(
                           Translations.of(context).strUpdateServiceStatus,
                        ...
                        Container(
                          ...
                          child: Padding(
                            ...
                            child: DropdownButton<String>(
                              ...
                              items: <String>[
                                Translations.of(context).strServiceAccepted,
                                Translations.of(context).strServiceInProgress,
                                Translations.of(context).strServiceCompleted,
  ...
  Future<void> wsUpdateBookingStatus() async {
    ...
    if(dropdownValue == Translations.of(context).strServiceAccepted) {
      map["status"] = Constants.serviceStatusAccepted;
    }else if(dropdownValue == Translations.of(context).strServiceInProgress) {
      map["status"] = Constants.serviceStatusInProgress;
    }else if(dropdownValue == Translations.of(context).strServiceCompleted) {
      map["status"] = Constants.serviceStatusCompleted;
    }
    APICall(context).updateBookingStatusApi(map, this);
  }
  ...
  void onResponseReceived(response, int requestCode) {
     if (requestCode == API.requestUpdateBookingStatus && this.mounted) {
      setState(() {
        _isShowLoader = false;
      });
